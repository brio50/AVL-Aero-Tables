...
Each sweep creates a timestamped subdirectory containing `results_total.{csv,json}`, a `provenance.json` record (git commit, dirty flag, source directory, snapshot path), AVL input snapshots in `.in/`, and raw `.st` output files in `.raw/` — so previous runs are never overwritten and every result is fully traceable. See [Directory Layout](user/concepts.md#directory-layout) for the full directory structure.
