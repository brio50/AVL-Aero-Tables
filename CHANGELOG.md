...
### Fixed
- `avl_fileread` no longer silently drops `AIRFOIL` inline coordinate tables and `DESIGN` variable declarations; both are now parsed into new `AvlSectionEntry` fields (`airfoil_coords`, `airfoil_x1`/`airfoil_x2`, `design`) (#15)
- `AFIL`/`NACA` keyword lines with the optional `X1 X2` chord-range params (e.g. `AFIL 0.0 1.0`) are now recognized; previously the airfoil file reference was silently dropped

## [2.0.1] - 2026-05-29

### Changed
- Plot figure titles and subplot labels improved: plain-English titles, Force/Moment distinction, full perturbation-variable names, `.avl` surface names instead of internal keys (#8)
- Doc cleanup in Quickstart and Concepts
