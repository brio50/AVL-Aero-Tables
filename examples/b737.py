...
def _save_html(fig: object, run_path: Path, docs_path: Path, write_docs: bool) -> None:
    # Run copy: self-contained (includes plotly CDN JS) so it opens standalone
    run_path.write_text(fig.to_html(include_plotlyjs="cdn", full_html=False))  # type: ignore[attr-defined]
    print(f"  → {run_path.name}")
    if write_docs:
        # Docs copy: fragment only — Sphinx loads plotly CDN once via html_js_files in conf.py
        docs_path.parent.mkdir(parents=True, exist_ok=True)
        docs_path.write_text(fig.to_html(include_plotlyjs="cdn", full_html=False))  # type: ignore[attr-defined]
